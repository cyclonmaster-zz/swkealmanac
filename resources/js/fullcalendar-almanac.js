/*!
 * FullAlmanac v2.2.6
 * Docs & License: http://arshaw.com/fullalmanac/
 * (c) 2013 Adam Shaw
 */
...
var fc = $.fullAlmanac = { version: "2.2.6" };
var fcViews = fc.views = {};


$.fn.fullAlmanac = function(options) {
	var args = Array.prototype.slice.call(arguments, 1); // for a possible method call
	var res = this; // what this function will return (this jQuery object by default)

	this.each(function(i, _element) { // loop each DOM element involved
		var element = $(_element);
		var calendar = element.data('fullAlmanac'); // get the existing calendar object (if any)
		...
		if (typeof options === 'string') {
			if (calendar && $.isFunction(calendar[options])) {
				...
				if (options === 'destroy') { // for the destroy method, must remove Calendar object data
					element.removeData('fullAlmanac');
				}
			}
		}
		// a new calendar initialization
			...
			element.data('fullAlmanac', calendar);
			calendar.render();
		}
	});
...
// TODO: document the structure and ordering of a FullAlmanac lang file
...
fc.datepickerLang = function(langCode, dpLangCode, dpOptions) {
	...
	// get the FullAlmanac internal option hash for this language. create if necessary
	...
	if ($.datepicker) {
		...
		// FullAlmanac and MomentJS use language codes like "pt-br" but Datepicker
...
// Sets FullAlmanac-specific translations. Will set the language as the global default.
fc.lang = function(langCode, newFcOptions) {
	...
	// set it as the default language for FullAlmanac
...
/* FullAlmanac-specific DOM Utilities
----------------------------------------------------------------------------------------------------------------------*/
...
/* FullAlmanac-specific Misc Utilities
----------------------------------------------------------------------------------------------------------------------*/
...
function makeMoment(args, parseAsUTC, parseZone) {
	...
	mom._fullAlmanac = true; // flag for extended functionality
...
newMomentProto.clone = function() {
	...
	if (this._fullAlmanac) {
		mom._fullAlmanac = true;
	}
...
newMomentProto.time = function(time) {
	...
	// Fallback to the original method (if there is one) if this moment wasn't created via FullAlmanac.
...
newMomentProto.format = function() {
	if (this._fullAlmanac && arguments[0]) { // an enhanced moment? and a format string provided?
...
// Require all HTML5 data-* attributes used by FullAlmanac to have this prefix.
// A value of '' will query attributes like data-event. A value of 'fc' will query attributes like data-fc-event.
fc.dataAttrPrefix = '';

// Given a jQuery element that might represent a dragged FullAlmanac event, returns an intermediate data structure
...
function Calendar(element, instanceOptions) {
	...
	function getViewSpec(requestedViewType) {
		...
		while (viewType && !viewClass) {
			...
			processSpecInput(fcViews[viewType]); // $.fullAlmanac.views, lower precedence
